...
export class BlogPostEditorComponent implements OnInit {
  ...
  updateLocalContentValue(): void {
    this.blogPostUpdateService.setBlogPostContent(this.blogPostData.content);
    this.contentEditorIsActive = false;
  }
  ...
  updateBlogPostData(isBlogPostPublished: boolean): void {
    this.blogPostUpdateService.setBlogPostTags(this.blogPostData.tags);
  ...
  openContentEditor(): void {
    this.contentEditorIsActive = true;
  }

  getDateStringInWords(naiveDateTime): string {
    return dayjs(
