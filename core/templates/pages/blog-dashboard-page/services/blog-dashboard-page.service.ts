...
export class BlogDashboardPageService {
  ...
  private _setActiveTab(hash: string) {
    if (hash.startsWith('#/blog_post_editor')) {
      ...
      this._blog_post_id = this.urlService.getBlogPostIdFromUrl();
    } else {
  ...
  navigateToEditorTabWithId(blogPostId): void {
  ...
  navigateToMainTab(): void {
    this.windowRef.nativeWindow.location.hash = this._DRAFT_TAB_URL;
    this._setActiveTab(this._DRAFT_TAB_URL);
  }
  ...
  deleteBlogPost(): void {
    this.blogPostEditorBackendService.deleteBlogPostAsync(this._blog_post_id).then(
      () => {
