...
export class SchemaBasedListEditorComponent {
  ...
  constructor(
    ...
    private idGenerationServie: IdGenerationService,
  ...
  ngOnInit(): void {
    this.baseFocusLabel = (
      ...
      this.idGenerationServie.generateNewId() + '-'
    );
  }

  getFocusLabel(index: number): void {
    ...
    return (
            index === 0 ? this.baseFocusLabel : this.baseFocusLabel +
            index.toString());
  }

  hasDuplicates(): boolean {
    let valuesSoFar: object = {};
    for (let i = 0; i < this.localValue.length; i++) {
      let value = this.localValue[i];
...
angular.module('oppia').directive('schemaBasedListEditor',
  downgradeComponent({ component: SchemaBasedListEditorComponent }));
